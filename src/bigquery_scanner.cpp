...
static void BigQueryScan(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
    ...
    if (read_rows_response.ok()) {
      auto record_batch =
          BigQueryResult::GetArrowRecordBatch(read_rows_response->arrow_record_batch(), schema);
	  ...
	  for (idx_t c = 0; c < output.ColumnCount(); c++) {
		...
		idx_t max_rows = std::min(
			static_cast<int64_t>(record_batch->num_rows()),
		...
		for (r = 0; r < max_rows; r++) {
			...
			switch (scalar->type->id()) {
				...
				output.SetValue(c, r, Value::BIGINT(std::dynamic_pointer_cast<arrow::Int64Scalar>(scalar)->value));
				break;
				case arrow::Type::STRING: {
				auto v = std::dynamic_pointer_cast<arrow::StringScalar>(scalar)->view();
				output.SetValue(c, r, Value(string(v)));
				}
				break;
				default:
				std::cout << std::left << std::setw(15) << "UNDEFINED ";
				output.SetValue(c, r, Value());
			}
		}
	  }
    }
